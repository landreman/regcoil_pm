subroutine regcoil_read_bnorm()
  ...
  if (allocated(Bnormal_from_TF_and_plasma_current)) deallocate(Bnormal_from_TF_and_plasma_current)
  allocate(Bnormal_from_TF_and_plasma_current(ntheta_plasma,nzeta_plasma),stat=iflag)
  if (iflag .ne. 0) stop 'regcoil_read_bnorm Allocation error!'

  Bnormal_from_TF_and_plasma_current = 0

!!$  if (.not. load_bnorm) then
!!$     if (verbose) print *,"Not reading a bnorm file, so Bnormal_from_TF_and_plasma_current arrays will all be 0."
!!$     return
!!$  end if
  ...
  if ( nbf > 0 ) then
     ...
     do izeta = 1,nzeta_plasma
        do itheta = 1,ntheta_plasma
           do i = 1, nbf
              Bnormal_from_TF_and_plasma_current(itheta,izeta) = Bnormal_from_TF_and_plasma_current(itheta,izeta) + &
...
!!$  do izeta = 1,nzeta_plasma
!!$     do itheta = 1,ntheta_plasma
!!$        Bnormal_from_TF_and_plasma_current_1D((izeta-1)*ntheta_plasma+itheta) = Bnormal_from_TF_and_plasma_current(itheta,izeta)
!!$     end do
!!$  end do
!!$  ! The syntax in the next line might be faster? But I should check the order.
!!$  !Bnormal_from_TF_and_plasma_current_1D = reshape(Bnormal_from_TF_and_plasma_current, (/ntheta_plasma*nzeta_plasma/))
  ...
  if (include_Bnormal_from_TF) then
     if (verbose) print *,"Adding Bnormal from a 1/R toroidal field."
     ! B_normal = (mu0 G / (2 pi R)) * (n dot e_zeta),
     ! e_zeta = -sin(zeta) e_x + cos(zeta) e_y, and e_* has unit length.
     ! The sign convention is that a positive net_poloidal_current_Amperes gives a B_zeta in the + e_zeta direction.
     do izeta = 1, nzeta_plasma
        Bnormal_from_TF_and_plasma_current(:,izeta) = Bnormal_from_TF_and_plasma_current(:,izeta) &
          + mu0 * net_poloidal_current_Amperes / (2 * pi * sqrt(r_plasma(1,:,izeta) ** 2 + r_plasma(2,:,izeta) ** 2)) &
          * (-sin(zeta_plasma(izeta)) * normal_plasma(1,:,izeta) + cos(zeta_plasma(izeta)) * normal_plasma(2,:,izeta)) / norm_normal_plasma(:,izeta)
     end do
  end if
