subroutine regcoil_Chebyshev_grid(N, xMin, xMax, x, weights, D)
  ...
  real(dp), intent(out) :: x(N), weights(N), D(N,N)

  integer :: i, j, N1, M
  real(dp), parameter :: pi = 3.1415926535897932384626433d+0
  real(dp), allocatable :: c(:), bigX(:,:), dX(:,:), sumD(:), cw(:), cc(:), f(:)
...
end subroutine Regcoil_Chebyshev_grid
