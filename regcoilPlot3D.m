regcoil_out_filename = 'examples/NCSX_vv_randomResolution1/regcoil_out.NCSX_vv_randomResolution1.nc';
...
d_initial = ncread(regcoil_out_filename,'d_initial');
...
d = d_initial;
r_coil_outer = r_coil + sign_normal * d * unit_normal_coil;
...
surf(squeeze(r_coil(1,:,:)), squeeze(r_coil(2,:,:)), squeeze(r_coil(3,:,:)),abs_M_inner,'edgecolor','none','facecolor','interp','facealpha',0.7)
surf(squeeze(r_coil_outer(1,:,:)), squeeze(r_coil_outer(2,:,:)), squeeze(r_coil_outer(3,:,:)),abs_M_outer,'edgecolor','none','facecolor','interp','facealpha',0.1)
colorbar
lighting gouraud
...
for js = 1:ns_magnetization
    X(:,:,js) = r_coil(1,1:end-1,:) + sign_normal * d * s_magnetization(js) * unit_normal_coil(1,:,:);
    Y(:,:,js) = r_coil(2,1:end-1,:) + sign_normal * d * s_magnetization(js) * unit_normal_coil(2,:,:);
    Z(:,:,js) = r_coil(3,1:end-1,:) + sign_normal * d * s_magnetization(js) * unit_normal_coil(3,:,:);
end
scale = 2;
